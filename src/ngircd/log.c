...
/* Sets highest log level to show. LOG_CRIT is probably a good level for slow PCs.
 *
 * Added this so users can have the console not be flooded with unnecessary messages.
 * This can get quite slow on old 8088s with CGA.
 */
int consoleLevel = LOG_DEBUG;

static void
Log_Message(int Level, const char *msg)
{
        if (!Is_Daemon && (Level <= consoleLevel)) {
                /* log to console */
                fprintf(stdout, "[0:%d %4ld] %s\n", Level,
