...
#ifdef IO_USE_POLL
static int
io_dispatch_poll(struct tm *tv)
{
        time_t sec = tv->tm_sec * 1000;
        int i, ret, timeout = sec;
        int fds_ready;
        short what;
...
#endif
#ifdef IO_USE_POLL
        return io_dispatch_poll((void*)tv);
#endif
        return -1;
}
