...
/*
 * mTCP wrapper stuff... lazy. Should move these to mtcpwrap.cpp
 */
  
void mtcp_cb_clientserver(int sock, short what) {
        cb_clientserver(sock, what);
}

int mtcp_New_Connection(int Sock, bool IsSSL) {
    return New_Connection(Sock, IsSSL);
}
        ...
        for(idx = 0; idx < Pool_Size; idx++) {
                if(My_Connections[idx].sock > NONE &&
                   My_Connections[idx].sock != ExceptOf)
                        closesocket2(My_Connections[idx].sock);
        }
}
...
Conn_InitListeners( void )
{
        ...
        if (count > 0) {
                ...
                for (i = 0; i < count; i++) {
                        ...
                        getsockname(fd, addr.sin4.sin_addr,
                                    (size_t*)&addr_len);
...
//#ifdef XCHOP
		...
		//printf("Asking to bind on port %u\n", Port);
        if (bind(sock, Port) != 0) {
...
Conn_Handler(void)
{
		...
		for (i=0; i<Pool_Size; i++) {
			My_Connections[i].rbuf.mem = NULL;
			My_Connections[i].wbuf.mem = NULL;
		}
        ...
        while (!NGIRCd_SignalQuit && !NGIRCd_SignalRestart) {
                ...
                if (kbhit()) {
                    if (getch() == 27) {
                        Log(LOG_NOTICE, "Escape pressed, raising SIGQUIT.");
...
/*                for (i = 0; i < Pool_Size; i++) {
                ...
                //tv.tv_usec = 0;
                tv.tm_sec = command_available ? 0 : 1;

                /* Wait for activity ... */
/*                i = io_dispatch((void*)&tv);
                if (i == -1 && errno != EINTR) {
...
Conn_Write( CONN_ID Idx, char *Data, size_t Len )
{
...
#endif
        {
				...
				//TODO: This used to be an if statement. This is a hack to get around the
				//low memory we have in DOS.
				//It SHOULD be safe. It shouldn't ever have to send more than ~2 KB in a
				//loop, and a connection problem in Handle_Write will cause it to exit the loop.
				int i;
				for (i=0; i<25; i++)
                ...
                if (array_bytes(&My_Connections[Idx].wbuf) + Len >=
                    writebuf_limit) {
                        Log(LOG_NOTICE,
                            "Write buffer space exhausted (connection %d, limit is %u bytes, %u bytes new, %u bytes pending)",
...
Conn_Close(CONN_ID Idx, const char *LogMsg, const char *FwdMsg, bool InformClient)
{
        ...
        if (InformClient) {
                ...
                if ((c != NULL) && (Client_Type(c) == CLIENT_USER)) {
                        Conn_WriteStr( Idx,
                         ":%s NOTICE %s :%sConnection statistics: client %l kb, server %1 kb.",
                         Client_ID(Client_ThisServer()), Client_ID(c),
                         NOTICE_TXTPREFIX,
                         My_Connections[Idx].bytes_in / 1024,
                         My_Connections[Idx].bytes_out / 1024);
                }
#endif
...
#endif
        /* Shut down socket */
        if (! closesocket(My_Connections[Idx].sock)) {
                /* Oops, we can't close the socket!? This is ... ugly! */
                /*Log(LOG_CRIT,
                    "Error in closesocket() connection %d (socket %d) with %s:%u - %s! (ignored)",
                    Idx, My_Connections[Idx].sock, My_Connections[Idx].host,
                    port, strerror(errno));*/
        }
        ...
        if (!io_close(My_Connections[Idx].sock)) {
            /* Oops, we can't close the socket!? This is ... ugly! */
            /*Log(LOG_CRIT,
                "Error in io_close() connection %d (socket %d) with %s:%u - %s! (ignored)",
                Idx, My_Connections[Idx].sock, My_Connections[Idx].host,
                port, strerror(errno));*/
        }
		...
		/* NOTE: I got rid of this stuff being calculated with doubles.
		 * Open Watcom 1.9 has a clib bug where the FPU detection routine
		 * locks an original 8086/8088 with a WAIT. Without an 8087
		 * installed, the bus control is never returned back to the CPU.
		 * The system hangs. If you have an 8087, it works, but most
		 * people don't have one.
		 */

        /* Calculate statistics and log information */
        in_k = My_Connections[Idx].bytes_in / 1024;
        out_k = My_Connections[Idx].bytes_out / 1024;
#ifdef ZLIB
        if (Conn_OPTION_ISSET( &My_Connections[Idx], CONN_ZIP)) {
                in_z_k = My_Connections[Idx].zip.bytes_in / 1024;
                out_z_k = My_Connections[Idx].zip.bytes_out / 1024;
                ...
                Log(LOG_INFO,
                    "Connection %d with \"%s:%d\" closed (in: %1dk/%ldk/%d%%, out: %1dk/%1dk/%d%%).",
        ...
        {
                Log(LOG_INFO,
                    "Connection %d with \"%s:%u\" closed (in: %1dk, out: %1dk).",
                    Idx, My_Connections[Idx].host, port,
                    in_k, out_k);
        }
...
Handle_Write( CONN_ID Idx )
{
...
#endif
        {
			//wdatalen = (wdatalen > 128) ? 128 : wdatalen;
                len = send(My_Connections[Idx].sock,
                            array_start(&My_Connections[Idx].wbuf), wdatalen, 0 );
                /*{ //HACK ALERT
                    int j;
                    for (j = 0; j < wdatalen; j++) {
                        printf("%c", ((char*)array_start(&My_Connections[Idx].wbuf))[j]);
                    }
                }*/
        }
        ...
        if( len < 0 ) {
                ...
                Conn_Close(Idx, "Write error", "Client closed connection",
                    false);
...
New_Connection(int Sock, UNUSED bool IsSSL)
{
		...
		new_sock = Sock;
		mtcp_getAddr(Sock, new_addr.sin4.sin_addr);
		new_addr.sin4.port = mtcp_getPort(Sock);
		
		//array_alloc(&My_Connections[new_sock].rbuf, 1, 4095);
		//array_alloc(&My_Connections[new_sock].wbuf, 1, 4095);
		
		//printf("rbuf = %u\n", My_Connections[new_sock].rbuf.allocated); 
		//printf("wbuf = %u\n", My_Connections[new_sock].wbuf.allocated); 
        ...
        if (!Init_Socket(new_sock)) {
            Log(LOG_ALERT, "Init_Socket failed on socket %d!",
                Sock);
            return -1;
        }
...
Check_Servers(void)
{
        ...
        for (i = 0; i < MAX_SERVERS; i++) {
				...
				New_Server(i, &Conf_Server[i].dst_addr[0]); //TODO: This is safe??
        }
} /* Check_Servers */
...
/*        if (res && bind(new_sock, (struct sockaddr *) &Conf_Server[Server].bind_addr,
		...
		res = connect(new_sock, Conf_Server[Server].host, Conf_Server[Server].port);
        if(( res != 0 )) { // && ( errno != EINPROGRESS )) { //TODO
		...
		cb_connserver(new_sock, IO_WANTWRITE);
} /* New_Server */
