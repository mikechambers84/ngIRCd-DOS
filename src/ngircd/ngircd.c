...
main(int argc, const char *argv[])
{
        ...
        Pidfile_Delete();
		shutdown(0); //mTCP shutdown

        return 0;
} /* main */

/*
void far* _dosmalloc(size_t bytes) {
    union REGS inregs, outregs;

    bytes = (bytes + 15) / 16; //convert to paragraphs

    inregs.h.ah = 0x48;
    inregs.x.bx = (unsigned int)bytes;

    _int86(0x21, &inregs, &outregs);

    if (outregs.flags & INTR_CF) { //fail
        return (void far*)NULL;
    }
    else { //success
        return (void far*)((unsigned long)outregs.x.ax << 16);
    }
}

void _dosrealloc(void far* ptr, size_t bytes) {
    union REGS inregs, outregs;
    ...
    inregs.h.ah = 0x48;
    inregs.x.bx = (unsigned int)bytes;
    inregs.x.es = (unsigned int)((unsigned long)ptr >> 16);
